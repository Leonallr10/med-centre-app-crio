describe('Hospital', () => {
  beforeEach(() => {
    cy.visit('http://localhost:3001'); // Use local development URL
  });
  ...
  it('should display all booked slots on the My Bookings page', () => {
    ...
    cy.visit('http://localhost:3001/#/my-bookings'); // Use local development URL with hash router
    cy.get('h1[data-testid="my-bookings-title"]').contains('My Bookings').should('be.visible');
  });
